"""Canonical gaze-to-saliency-mask conversion.

This is the single source of truth for ``GazeToMask``, which was previously
duplicated (identically) in ``gabril_utils.py``, ``scripts/gabril_utils.py`` and
``baselines/gaze/gaze_to_mask.py``. Behaviour is preserved verbatim from those
canonical copies (normalized Gaussian ``1 / (2*pi*sigma**2)``).

NOTE: two *drifted* variants still live in ``scripts/generate_and_visualize_masks.py``
(meshgrid Gaussian, no normalization) and ``scripts/generate_full_gaze_tensor.py`` /
``scripts/make_trajectory_video.py`` (subset, missing ``find_bunch_of_maps`` and a
different Gaussian). Those were intentionally NOT merged here because their maths
differs; they need a separate human decision before unifying.
"""
