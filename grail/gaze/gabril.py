"""GABRIL gaze dataset/eval helpers.

Consolidated single source of truth for what was previously duplicated verbatim
in ``gabril_utils.py`` (repo root) and ``scripts/gabril_utils.py``. Behaviour is
preserved; the only changes are:

  * ``GazeToMask`` is now imported from :mod:`grail.gaze.gaze_to_mask` instead of
    being redefined inline.
  * ``FrameWriter`` and ``apply_gmd_dropout`` are imported lazily inside the
    functions that use them (they were previously module-level imports, one of
    which — ``frame_writer`` — does not exist on the path, making the original
    modules un-importable). Lazy import makes this module importable while
    keeping identical runtime behaviour for ``load_dataset`` / ``evaluate``.
"""
