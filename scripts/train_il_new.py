...
def main():
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(agent.optimizer, mode='max', factor=0.5, patience=args.lr_patience)
    ...
    for epoch in range(args.epochs):
        ...
        val_samples = 0
        with torch.no_grad():
            for states, actions, gazes in val_dataloader:
                states = states.to(device)
                actions = actions.to(device)
                gazes = gazes.to(device)
                
                probs = agent.model(states, gazes) if args.use_gaze else agent.model(states, None)
        ...
        print(f"Epoch {epoch+1}/{args.epochs} Avg Loss: {avg_loss:.4f} | Train Acc: {train_acc:.3f} | Val Acc: {val_acc:.3f}")
        ...
        if val_acc > best_val_acc:
            ...
            best_save_path = os.path.join(save_dir, f"new_{args.env}_{args.rules}_il_best_{gaze_str}.pth")
            agent.save(best_save_path)
            print(f"*** New best model saved to {best_save_path} (ValAcc: {val_acc:.3f})")
        else:
            early_stop_patience_counter += 1
            print(f"Patience {early_stop_patience_counter}/{args.patience}")
            if early_stop_patience_counter >= args.patience:
                print(f"Early stopping triggered after {early_stop_patience_counter} epochs without val_acc improvement.")
                break
    ...
    print("="*50)
    print("Epoch | Mean Score | Std Dev | Loss   | Train Acc | Val Acc")
    for res in results_log:
        print(f"{res['epoch']:5d} | {res['mean_reward']:10.2f} | {res['std_reward']:7.2f} | {res['loss']:.4f} | {res['train_acc']:9.3f} | {res['val_acc']:7.3f}")
    print("="*50)
