...
def worker_fn(indices, env_name):
    ...
    for i in range(len(subset_df)):
        try:
            ...
            print(f"Error processing frame {current_fid}: {e}")
            # import traceback
            # traceback.print_exc()
            pass
...
def main():
    ...
    tasks = [(chunk, args.env) for chunk in chunks]
    
    with ctx.Pool(args.num_workers) as pool:
        results_nested = []
        # Use starmap
        for res in tqdm(pool.starmap(worker_fn, tasks), total=len(chunks)):
