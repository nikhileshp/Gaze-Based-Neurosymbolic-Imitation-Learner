...
def parse_args():
    ...
    parser.add_argument("--env-id", type=str, default="Freeway-v4",
...
def make_env(env_id, seed, idx, capture_video, run_name):
    def thunk():
        ...
        if capture_video:
            if idx == 0:
                env = gym.wrappers.RecordVideo(env, f"videos/{run_name}", video_length=2)
...
if __name__ == "__main__":
    ...
    agent = pickle.load(open("ppo_checkpoints/Freeway-v4_s0_step9984K.pkl", "rb"))
    ...
    next_obs = torch.Tensor(env.reset()).to(device).unsqueeze(0)
    next_done = torch.zeros(args.num_envs).to(device).unsqueeze(0)
