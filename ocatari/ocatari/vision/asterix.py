...
def _detect_objects(objects, obs, hud=False):
    ...
    # All consumable types share slots 17-24. We must aggregate all detections
    # before calling match_objects ONCE, otherwise each call clears prior results.
    all_consumable_bb = []
 
    cauldron = find_mc_objects(
        obs, objects_colors["cauldron"], closing_dist=2, size=(7, 10), tol_s=2,
        miny=24, maxy=151)
    all_consumable_bb.extend(cauldron)
 
    # Helmet: only the grey color [170,170,170] reliably appears in-game.
    # The other 4 defined colors (240/236/214/192) are never rendered.
    helmet = find_objects(obs, [170, 170, 170], closing_dist=1, min_distance=1,
                          size=(7, 11), tol_s=3, miny=24, maxy=151)
    all_consumable_bb.extend(helmet)
 
    shield = find_objects(obs, objects_colors["shield"], closing_dist=1, size=(5, 11), tol_s=1, min_distance=2,
                          miny=24, maxy=151)
    all_consumable_bb.extend(shield)
 
    lamp = find_mc_objects(obs, objects_colors["lamp"], closing_dist=4, size=(
        8, 11), tol_s=1, miny=24, maxy=151)
    all_consumable_bb.extend(lamp)
 
    apple = find_mc_objects(obs, objects_colors["apple"], closing_dist=2, min_distance=2, size=(8, 11),
                            tol_s=1, miny=24, maxy=151)
    all_consumable_bb.extend(apple)
 
    fish = find_objects(obs, objects_colors["fish"], closing_dist=1, min_distance=1,
                        size=(8, 5), tol_s=2, miny=24, maxy=151)
    all_consumable_bb.extend(fish)
 
    meat = find_mc_objects(obs, objects_colors["meat"], closing_dist=1, min_distance=1, size=(5, 11),
                           tol_s=2, miny=24, maxy=151)
    if len(shield) == 0:
        all_consumable_bb.extend(meat)
 
    mug = find_mc_objects(obs, objects_colors["mug"], closing_dist=2, min_distance=2, size=(7, 11),
                          tol_s=1, miny=24, maxy=151)
    all_consumable_bb.extend(mug)
 
    # Single match_objects call with all consumable bounding boxes
    # Use a generic Consumable class; Cauldron is used since it inherits from GameObject
    match_objects(objects, all_consumable_bb, 17, 8, Cauldron)
