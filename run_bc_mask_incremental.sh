...
RUN_DIR="models/bc/bc_mask_incremental_20ep"
...
conda run -n nesy-il python -u train_bc_pt.py \
    ...
    --lr_patience 0 \
    --use_gazemap \
    --run_dir "$RUN_DIR" > log_bc_mask_incremental_full.log 2>&1 &

echo "Started training in the background on GPU 2."
echo "Tail the log file using: tail -f log_bc_mask_incremental_full.log"
