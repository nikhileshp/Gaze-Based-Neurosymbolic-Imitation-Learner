...
class NudgeBaseEnv(ABC):
    ...
    def map_action(self, model_action) -> int:
        ...
        if self.mode == 'ppo':
            ...
            # Extract action from first segment of clause name (e.g. "up_dodge_right" -> "up")
            action_key = model_action.split("_")[0]
            if action_key in self.pred2action:
                return self.pred2action[action_key]
            raise ValueError(f"Invalid predicate '{model_action}' provided. "
                             f"Extracted key '{action_key}' not in {list(self.pred2action.keys())}.")
