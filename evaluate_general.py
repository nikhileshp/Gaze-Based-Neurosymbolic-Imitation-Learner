...
def main():
    ...
    parser.add_argument("-m", "--modifs", nargs="+", default=[], help="List of modifications to apply to HackAtari")
    ...
    print(f"Initializing Environment: {args.env} with modifs: {args.modifs}...")
    if args.modifs:
        env = NudgeBaseEnv.from_name(args.env, mode='logic', modifs=args.modifs)
    else:
    ...
    for ep in range(1, args.num_episodes + 1):
        ...
        search_pattern_1 = os.path.join(folder_path, f"*epoch_28_*.pth")
    ...
    if results:
        ...
        modifs_str = "_".join(args.modifs) if args.modifs else "no_modifs"
        csv_filename = f"{args.env}_{args.rules}_lr_{args.lr}_{gaze_dir}_{modifs_str}_eval_results.csv"
